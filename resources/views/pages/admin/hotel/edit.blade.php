    ...
    <div class="main-container container">
        <div class="row justify-content-center">
            <div class="col-lg-6 col-md-8 col-sm-12">
                ...
                <form class="form-horizontal" method="POST" action="{{ route('admin.hotel.update',$item->id) }}" enctype="multipart/form-data">
                    <div class="card text-white bg-dark mb-3">
                        <div class="card-header">
                            <h2 class="m-0">Edit Store</h2>
                        </div>
                        <div class="card-body">
                            {{ csrf_field() }}
                            {{ method_field('patch') }}
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="mobile" class="form-control" id="mobile" value="{{ old('mobile', $item->mobile) }}" minlength="10" maxlength="13" required autofocus>
                            </div>
                            <!-- /mobile -->
                            <!-- address -->
                            <div class="form-group">
                                ...
                                <select name="hotel_category" id="hotel_category" class="form-control" required>
                                    ...
                                    @if(isset($hotelCategoryList))
                                        @foreach($hotelCategoryList as $hotelCategory)
                                            <option value="{{ $hotelCategory->id }}" {{ ($hotelCategory->id == old('hotel_category', $item->hotel_category))? 'selected': '' }}>{{ $hotelCategory->category_name }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
