    ...
    <div class="main-container container">
        <div class="row justify-content-center">
            <div class="col-lg-6 col-md-8 col-sm-12">
                ...
                <form class="form-horizontal" method="POST" action="{{ route('admin.hotel.menu-item.update', ['hotelId'=> $hotelId, 'id'=>$item->id]) }}" enctype="multipart/form-data">
                    <div class="card text-white bg-dark mb-3">
                        <div class="card-header">
                            <h2 class="m-0">Edit Store Product</h2>
                        </div>
                        <div class="card-body">
                            {{ csrf_field() }}
                            {{ method_field('patch') }}
                                ...
                                <select name="item-category" id="item-category" class="form-control" required>
                                    ...
                                    @if(isset($itemCategoryList))
                                        @foreach($itemCategoryList as $itemCategory)
                                            <option value="{{ $itemCategory->id }}" {{ ($itemCategory->id == old('item-category', $item->item_category))? 'selected': '' }}>{{ $itemCategory->category_name }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="item-name" class="form-control" id="item-name" value="{{ old('item-name', $item->item_title) }}" required autofocus>
                            </div>
                            <!-- /item name -->
                            <!-- item ingredients -->
                            <div class="form-group">
                                <label class="col-form-label" for="item-ingredients">Item Ingredients (Optional)</label>
                                <textarea name="item-ingredients" class="form-control" id="item-ingredients">{{ old('item-ingredients', $item->item_ingredients) }}</textarea>
                            </div>
                            <!-- /item ingredients -->
                            <!-- item price -->
                            <div class="form-group">
                                <label class="col-form-label" for="item-price">Item Price (in Rupees)</label>
                                <input type="text" name="item-price" class="form-control" id="item-price" value="{{ old('item-price', $item->item_price) }}" required autofocus>
                            </div>
                            <!-- /item price -->
                            <!-- item photo -->
